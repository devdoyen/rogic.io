-- Delete references in histories first to avoid foreign key constraint violations
DELETE FROM histories WHERE stage_id IN (SELECT id FROM stages WHERE name IN ('Cross Ruby', 'Smile Face', 'Checkerboard 20x20', 'Giant Cross 30x30'));
