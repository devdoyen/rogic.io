...
- **해결 내역**:
  ...
  - **SLA 연산 수식 LaTeX 수식화**: README.md 부록 1의 PromQL 연산 명세를 기존 텍스트 나열 포맷에서 가독성이 뛰어난 LaTeX 수학 공식 블록(`$$`) 및 `promql` 코드 블록으로 구조화하여 전문성을 강화함. 또한 LaTeX 렌더러가 언더바(`_`) 기호 파싱 오류(`'_' allowed only in math mode`)를 뱉는 것을 방지하기 위해 기호($P_t$) 매핑 정의를 신설하여 수식을 무결하게 표준화하고, 수식 줄바꿈이 정상 작동하도록 마크다운 인덴트(들여쓰기)를 걷어내고 독립 블록으로 완전히 격리하여 가시성 오류를 조치함.
