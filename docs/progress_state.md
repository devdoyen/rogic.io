...
- **해결 내역**:
  ...
  - **야간 스테이징 인프라 자동 종료 비활성화**: AI 생성 퍼즐 기능 테스트 및 운영 모니터링을 지속할 수 있도록, 매일 새벽 2시(KST)에 AWS EC2 인스턴스를 자동으로 중단하던 GitHub Actions 스케줄러([staging-cleanup.yml](file:///.github/workflows/staging-cleanup.yml))의 cron 트리거(`schedule:`)를 주석 처리하여 비활성화 완료 (수동 실행용 `workflow_dispatch` 버튼은 유지).

### 스테이지 리스트 내 정답률/시간 비활성화 및 크기 라벨 정렬 개선 - 완료
- **현상**:
  - 퍼즐 선택 드롭다운 리스트 내 각 스테이지 카드 하단에 표시되는 정답률(Rate) 및 평균 소요 시간(⏱️) 정보가 레이아웃을 복잡하게 만듦.
  - 퍼즐 크기 라벨이 카드 이름 뒤에 제각각 다른 가로폭으로 비대칭하게 붙어 정렬이 깔끔하지 못함.
- **해결 내역**:
  - **정답률 및 시간 정보 제거**: [App.vue](file:///c:/Users/82107/dev/project/nemologic/frontend/src/App.vue)의 스테이지 드롭다운 카드 내 `stage-card-stats` 영역을 전면 제거하여 스테이지 카드 레이아웃을 깔끔하게 단순화함.
  - **크기 라벨 전면 배치 및 너비 고정**: [App.vue](file:///c:/Users/82107/dev/project/nemologic/frontend/src/App.vue)의 스테이지 목록 드롭다운 카드에서 크기 라벨(`stage-card-size-label`)을 퍼즐 이름의 좌측(앞)으로 이동하고, `width: 52px; text-align: center; flex-shrink: 0;` 속성을 부여하여 여러 사이즈(`5x5`, `10x10`, `15x15` 등)에 관계없이 크기 캡슐이 수직으로 일정한 폭으로 아름답게 정렬되도록 개선 완료.
  - **테스트 케이스 검증**: Vitest 기반의 기존 68개 모든 단위 테스트가 이상 없이 100% 성공 통과함을 확인 완료.

### AI 자동 생성 로컬 테스트용 스케줄러 시간대 및 크기 설정 변경 - 완료
- **현상**:
  - 기존 AI 자동 생성 스케줄러가 매일 04:17 UTC (KST 13:17)로 설정되어 있어 로컬 개발 및 기능 검증을 즉각 수행하기 어려움.
- **해결 내역**:
  - **스케줄러 설정 임시 변경**: [DailyPuzzleScheduler.java](file:///c:/Users/82107/dev/project/nemologic/backend/src/main/java/com/devdoyen/nemologic/scheduler/DailyPuzzleScheduler.java)의 `generateDailyPuzzle` 실행 크론 주기를 `"0 * * * * ?"` (매 분 0초마다 실행)로 당기고, 빈 버퍼 초기 기동 시 6가지 전체 사이즈(`5, 10, 15, 20, 25, 30`) 각각에 대해 5개씩 총 30개의 AI 퍼즐이 정상적으로 생성되고 DB에 저장되는지를 확인하기 위해 사이즈 배열을 원복함.
  - **API Rate Limit 방지 지연 시간 도입**: 30개의 대량 AI 퍼즐 생성 시 Gemini API 무료 티어(15 RPM) 한계로 인해 발생하는 `429 Too Many Requests` 예외를 해결하기 위해, 각 퍼즐 생성 및 루프 사이즈 간에 `Thread.sleep(5000)` (5초 지연)을 배치하여 안정적으로 대량 생성이 가능하도록 보완함.
  - **무료 티어 할당량 최적화 모델 교체**: 일일 할당량이 20 RPD로 극히 제한되어 있는 `gemini-2.5-flash` 대신, 무료 티어 기준 일일 할당량이 500 RPD로 훨씬 넉넉한 `gemini-3.1-flash-lite` 모델로 기본 AI 모델을 교체함.

- **Flyway 마이그레이션 실패 해결**:
  - **현상**: 프로덕션 배포 시 `V3__clean_up_old_stages.sql` 마이그레이션 중 외래키 제약조건(`fkk7t9a00lti7i5qc0owqx62qin`) 위반으로 배포가 실패함. (`histories` 테이블이 삭제 대상 `stages`를 참조하고 있었음)
  - **해결 내역**: 삭제 대상 스테이지들을 참조하는 `histories` 레코드를 먼저 제거하는 쿼리를 추가하여 제약조건 위반을 방지함.

### S3 + CloudFront CDN 프론트엔드 호스팅 개편 및 API 멀티 오리진 연동 - 완료
- **현상**: 기존에는 Nginx에서 정적 프론트엔드 파일과 백엔드 API 프록시를 하나의 EC2 인스턴스에서 동시 처리하여, 서버 스케일링이 비효율적이고 배포 시 Nginx 핫리로드 의존도가 높았음.
- **해결 내역**:
  - **AWS S3 정적 호스팅 및 OAC 도입**: 프론트엔드 웹 자산을 담을 S3 버킷을 환경별(Prod/Stage)로 구축하고 CloudFront OAC(Origin Access Control)를 설정하여 오직 CDN을 통해서만 보안 접근되도록 처리.
  - **CloudFront Multi-Origin 라우팅**: 단일 CDN 배포판에서 Default (`*`) 경로는 S3 버킷(정적 리소스)으로, `/api/*` 및 `/actuator/*` 경로는 EC2 백엔드 origin으로 분기 전송하도록 설정. API 통신은 캐시 비활성화(CachingDisabled) 및 헤더 투과(AllViewerExceptHostHeader) 정책 연동.
  - **ACM SSL 자동 발급**: `us-east-1` 리전에 `rogic.io` 및 `stage.rogic.io` 인증서를 자동 생성하도록 Terraform을 선언하고 검증 CNAME 레코드 추출이 가능하도록 outputs 추가.
  - **CI/CD 파이프라인 개편**: GitHub Actions 빌드 단계에서 더 이상 프론트엔드 도커 이미지를 제작하지 않도록 간소화하고, 대신 S3 파일 업로드(`aws s3 sync`) 및 CDN 캐시 무효화(`aws cloudfront create-invalidation`) 단계를 staging/prod 배포 과정에 적용.
  - **Nginx 경량화**: EC2의 Nginx 서비스 컨테이너를 공식 `nginx:alpine` 이미지 기반으로 교체하고 정적 파일 호스팅 설정을 전면 주석/제거 처리하여 순수 백엔드 API 라우터 역할로 최소화.

### S3 + CloudFront CDN 설정 주석 해제 및 복원 완료
- **현상**: ACM 인증서 발급 및 DNS 검증을 최초 1회 수행하기 위해 Terraform 구성에서 CloudFront 배포판 및 S3 버킷 정책을 임시로 주석 처리하였음.
- **해결 내역**:
  - DNS CNAME 검증 및 배포가 완료됨에 따라, `cdn.tf`에서 CloudFront distributions 및 S3 bucket policies 리소스의 블록 주석(`/* ... */`)을 해제함.
  - `outputs.tf`에서 `cloudfront_domain_prod`와 `cloudfront_domain_stage` 아웃풋의 출력값을 기존의 빈 문자열(`""`)에서 실제 배포판 도메인 네임(`.domain_name`)을 반환하도록 원복함.
  - `terraform validate` 검증을 완료하여 복원된 설정의 무결성을 확보함.

### ACM 인증서 자동 DNS 검증 및 CloudFront 의존성 개선 (Step 27) - 완료
- **현상**: ACM 인증서 생성 시 DNS 검증(CNAME 레코드 등록) 과정이 Terraform 흐름과 유기적으로 결합되어 있지 않아, 인증서가 `PENDING_VALIDATION` 상태인 와중에 CloudFront 배포판이 생성을 시도하다가 `InvalidViewerCertificate` 오류를 내고 배포가 실패하는 현상이 발생함.
- **해결 내역**:
  - `cdn.tf`에 `aws_route53_zone` 데이터 소스를 추가하여 `rogic.io` 호스팅 영역 정보를 동적으로 조회하도록 구성함.
  - `aws_route53_record.prod_cert_validation` 및 `aws_route53_record.stage_cert_validation` 리소스를 새롭게 도입하여, ACM 인증서 요청 시 생성되는 검증 CNAME DNS 레코드를 Route 53에 완전 자동 등록하도록 설정함.
  - `aws_acm_certificate_validation` 리소스를 `prod` 및 `stage` 환경 각각에 대해 선언하여, 인증서 발급이 온전히 완료(`ISSUED`)될 때까지 Terraform 실행 흐름을 일시 대기하도록 안전장치를 적용함.
  - `aws_cloudfront_distribution` 리소스의 `acm_certificate_arn` 속성을 ACM 인증서의 직속 ARN 대신 `aws_acm_certificate_validation` 리소스의 `certificate_arn` 출력값에 종속되도록 변경함으로써, 무조건 검증이 완료된 시점에만 CloudFront 배포가 시작되도록 배포 순서 정합성 문제를 완벽히 해결함.
  - 로컬 환경에서의 `terraform validate` 및 타겟 리소스 지정을 통한 `terraform plan` 드라이 런 검증을 성공적으로 마침.

### Route 53 도메인 레코드의 CloudFront Alias(별칭) 전환 (Step 28) - 완료
- **현상**: 인프라 배포 완료 후에도 `stage.rogic.io` 또는 `rogic.io` 접속 시 `nginx/1.31.2` 서버로부터 404 Not Found 에러가 지속 발생함. 이는 기존의 DNS A 레코드가 CloudFront 배포판 도메인이 아닌, EC2 인스턴스의 Elastic IP를 직접 가리켜서 발생한 문제로 판명됨. EC2 Nginx는 최신 아키텍처에 따라 정적 리소스 호스팅 설정을 걷어내고 순수 API 라우터 역할만 수행하므로 루트 경로 `/` 접근 시 404 에러를 응답함.
- **해결 내역**:
  - `cdn.tf`에 `aws_route53_record.prod_alias`, `aws_route53_record.prod_www_alias`, `aws_route53_record.stage_alias` 리소스를 신규 선언함.
  - 해당 레코드들의 타입을 `A` 레코드로 지정하고 `alias` 블록을 활용하여 각각 `prod_distribution` 및 `stage_distribution` CloudFront 도메인을 직접 가리키도록 설정함.
  - 기존 수동 등록 레코드와의 충돌을 방지하기 위해 `allow_overwrite = true` 설정을 부여함으로써 Terraform이 기존 레코드를 문제없이 덮어쓰고 관리할 수 있도록 구성함.
  - 로컬 환경에서의 `terraform validate` 검증을 완료하고, 타겟 리소스 지정을 통한 드라이 런 (`terraform plan`) 확인 결과 3개의 별칭 레코드가 정상 추가됨을 확인 완료함.

### 임시 80 포트 API 개방 및 호스트 자동 복구/백업 정책 고도화 (Step 29) - 완료
- **현상**: CloudFront를 통한 프론트엔드 접속 이후 백엔드 API 통신 실패 현상이 발생함. 이는 CloudFront가 EC2 Nginx에 80 포트(HTTP)로 요청을 전송하나, Nginx 80 포트 블록에서 모든 요청을 HTTPS로 301 강제 리다이렉트 처리하면서 호스트명 검증 에러가 발생한 원인임. 또한 호스트 가상머신 예기치 못한 중단 대비책 및 DB 백업 소실 범위 단축 필요성이 제기됨.
- **해결 내역**:
  - `nginx.stage.conf` 및 `nginx.prod.conf` 내의 `listen 80;` 서버 블록에 `/api/` 및 `/actuator/` 경로에 대한 `proxy_pass` 프록시 규칙을 추가하여, 해당 요청에 대해서는 HTTPS 리다이렉트를 건너뛰고 백엔드로 직접 전달하도록 수정 완료함.
  - `main.tf`에 `aws_cloudwatch_metric_alarm.ec2_auto_recovery_alarm` 리소스를 신설하여, 시스템 상태 검사 실패(`StatusCheckFailed_System`) 발생 시 자동으로 동일 EIP/EBS 설정을 유지한 채 건강한 물리 서버로 가상머신이 마이그레이션 및 자동 기동(Recover)되도록 구축함.
  - `main.tf`에 `aws_s3_bucket_lifecycle_configuration.backup_lifecycle` 리소스를 추가하여 S3 내 백업 데이터 누적으로 인한 비용 문제를 해소하기 위해 30일이 지난 백업 파일은 자동 영구 삭제되도록 설정함.
  - `playbook.yml` 내 DB 백업 크론탭 스케줄 설정을 기존 일 1회에서 **6시간 주기(00, 06, 12, 18시)**로 단축 조정하여 데이터 유실 시간 범위를 줄임.

### 백엔드 전용 도메인 신설 및 End-to-End HTTPS 구축 (Step 30) - 완료
- **해결 내역**:
  - `cdn.tf`에 `api.stage.rogic.io` 및 `api.rogic.io` 전용 Route 53 백엔드 A 레코드를 신설하여 각각 EC2 인스턴스의 Elastic IP를 직접 조준하도록 설정함.
  - `playbook.yml` 내 `certbot_domains` 속성에 백엔드 서브도메인을 추가하고 Nginx port 443 블록에 각 서브도메인을 매핑함.
  - 스테이징 및 운영 서버에 직접 SSL 인증서 확장을 적용하여 Multi-domain SSL 인증서(`stage.rogic.io`, `api.stage.rogic.io` / `rotagic.com`, `www.rotagic.com`, `rogic.io`, `www.rogic.io`, `api.rogic.io`) 발급 및 Nginx 연동을 성공적으로 마침.
  - CloudFront 배포판의 backend origin 설정을 `http-only`에서 `https-only`로 전환하고, 대상 도메인을 새롭게 신설된 백엔드 서브도메인(`api.stage.rogic.io` / `api.rogic.io`)으로 변경하여 E2E HTTPS 통신 구간 암호화를 완벽히 구축 완료함.
  - E2E HTTPS가 안착함에 따라 임시로 HTTP 80 포트에 열어두었던 `/api/` 및 `/actuator/` 경로의 프록시 설정을 완전히 삭제(폐쇄)하고, 다시 모든 HTTP 80 포트 유입 요청이 HTTPS 443 포트로 강제 301 리다이렉트되도록 보안 원복 처리를 마무리함.
...
### SSL 자동 갱신 훅 검증 및 구버전 인증서 정리 (Step 33) - 완료
- **해결 내역**:
...
- **해결 내역**:
  ...
  - **그리드 레이아웃 최적화**: 
    - 첫 번째 행(`Nemologic Service SLA Metrics`)을 24가로폭 그리드에 최적화하여 3개 열 배치(각 폭 8) 구조의 카드 행렬 레이아웃을 구성함.
...
- **해결 내역**:
  ...
  - **가이드라인 규칙 신설 및 문서 포맷 리팩토링**: 마크다운 문서 내 트러블슈팅 및 상세 소제목 기술 시 개행 표준화 및 절대 경로(`file:///`) 삽입 금지 조치를 포함한 신규 거버넌스 규칙 [documentation-guidelines.md](../.agents/rules/documentation-guidelines.md)를 제정함. 수치 대조 표(Table) 우선 적용 규칙을 추가하고 README.md에 비용 비교 테이블을 구현함. 또한, 개발 작업 완료 후의 커밋 메시지 컨벤션(Conventional Commits 규칙) 확립 및 로컬 커밋 자동 수행과 원격 푸시(Push) 권한 위임 거버넌스를 제정하는 신규 규칙 [git-and-commit-guidelines.md](../.agents/rules/git-and-commit-guidelines.md)를 추가 수립함. 아울러 에이전트 규칙 변경 사항을 대외적으로 공개하기 위해 **README.md에 `4.3. Custom Development Rules & Guidelines` 섹션을 신설**하여 규칙 목록과 설명 테이블을 명세함. 비용 최적화와 기술적 한계 분석의 층위 독립을 위해, 기존 `1.2` 하위로 구성되어 있던 기술적 타협 및 완화 방안을 **독립 섹션 `1.3. Technical Trade-offs & Mitigations`로 분리**하고, 이에 맞춰 하위 후속 장표 인덱스(`1.4` 네트워크, `1.5` 관제, `1.6` 트러블슈팅) 및 TOC 내부 앵커 링크들을 전면 재구성하여 가독성과 서술 깊이를 향상함.
