...
- **해결 내역**:
  ...
  - **문서화 가이드라인 규칙 추가 및 포맷 리팩토링**: 마크다운 문서 내 트러블슈팅 및 상세 항목 소제목 기술 시 개행 표준화 및 절대 경로(`file:///`) 삽입 금지 조치를 포함한 신규 거버넌스 규칙 [documentation-guidelines.md](../.agents/rules/documentation-guidelines.md)를 제정함. 수치 대조 표(Table) 우선 적용 규칙을 추가하고 README.md에 비용 비교 테이블을 구현함. 또한 비용 최적화(Cost Optimization) 이면에 상존하는 기술적 현실성과 한계점을 객관적으로 보존하기 위해, `1.2` 하위의 각 세부 항목마다 물리적/논리적 트레이드오프(SPOF 노출, RTO 지연, 빌드 성능 페널티) 및 이를 극복하기 위한 대응책(Auto Recovery, 복구 지향 아키텍처, 빌드 컴퓨팅 오프로딩)을 명시화하여 포트폴리오의 실무적 깊이를 보장함.
