...
## 2. 다음 목표 (Next Goals)
- **CNAME DNS 레코드 검증 및 실 연동**: 제공된 `acm_validation_dns_prod` 및 `acm_validation_dns_stage` DNS CNAME 레코드를 DNS 도메인 관리 콘솔에 등록하여 ACM SSL 인증 및 CloudFront 최종 연동 완료.
- **구도메인(rotagic.com) 301 리디렉션 구현**: `rotagic.com`에 대해 CloudFront 엣지 함수(CloudFront Functions)를 추가 생성하여 `https://rogic.io`로 안전하고 지연 없는 HTTPS 301 영구 리디렉션 처리 적용.
